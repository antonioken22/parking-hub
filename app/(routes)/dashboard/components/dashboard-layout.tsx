  ...
  return (
    ...
    <div className="flex flex-col w-full lg:w-2/3 bg-primary-foreground text-white rounded-lg mb-4 lg:mb-0 p-4 lg:p-8 mr-6 ml">
      <div className="flex justify-between mb-5 space-x-1">
        <button
          className={`flex-1 p-2 rounded ${selectedTab === 'logs' ? 'bg-black text-white' : 'bg-primary text-black'}`}
          onClick={() => setSelectedTab('logs')}
        >
          Logs
        </button>
        <button
          className={`flex-1 p-2 rounded ${selectedTab === 'vehicles' ? 'bg-black text-white' : 'bg-primary text-black'}`}
          onClick={() => setSelectedTab('vehicles')}
        >
          Vehicles
        </button>
        <button
          className={`flex-1 p-2 rounded ${selectedTab === 'delete' ? 'bg-black text-white' : 'bg-primary text-black'}`}
          onClick={() => setSelectedTab('delete')}
        >
          Delete
        </button>
        <button
          className={`flex-1 p-2 rounded ${selectedTab === 'configure' ? 'bg-black text-white' : 'bg-primary text-black'}`}
