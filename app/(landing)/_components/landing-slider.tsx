...
>
  {ServiceData.map((item) => (
    <SwiperSlide key={item.name} className="flex items-center justify-center">
      <div className="flex flex-col gap-4 mb-12 group relative shadow-lg text-white rounded-xl px-4 py-6 h-auto max-w-xs lg:h-[400px] lg:w-[350px] overflow-hidden cursor-pointer border-2 border-customOrange">
        ...
        <div className="relative flex flex-col gap-3 text-white group-hover:text-customOrange">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <div className="flex flex-col">
              {item.name && (
                <h2 className="text-base lg:text-lg font-semibold">{item.name}</h2>
              )}
        ...
        <RxArrowTopRight className="absolute bottom-4 left-4 w-6 h-6 text-white group-hover:text-customOrange group-hover:rotate-45 duration-100" />
      </div>
    </SwiperSlide>
  ))}
</Swiper>
